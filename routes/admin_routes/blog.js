...
router.post("/audit", async (req, res) => {
  try {
    ...
    await freeMail(
      user.NAME,
      user.EMAIL,
      `Hi, your post has been set to ${status} By and Admin <br> ${message}`,
      `You're blog post status has been updated`
    );
...
router.delete("/audit", async (req, res) => {
  try {
    ...
    await freeMail(
      user.NAME,
      user.EMAIL,
      `Hi, your post has been deleted, By an Admin`,
...
router.post("/up-new-blog/:id", upload.single("image"), async (req, res) => {
  try {
    ...
    if (!blog) return res.redirect(`/${ROLE}/blog`);
