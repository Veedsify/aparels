...
router.get("/", async (req, res) => {
  try {
    ...
    res.render("admin/order", { pendingOrders: response });
  } catch (err) {
    console.error(err);
  }
});
...
router.get("/delivered", async (req, res) => {
  try {
    ...
    res.render("admin/delivered-orders", { deliveredOrder: response });
  } catch (err) {
    console.error(err);
  }
});
...
router.get("/shipped", async (req, res) => {
  try {
    ...
    res.render("admin/shipped-orders", { shippedOrder: response });
  } catch (err) {
    console.error(err);
  }
});
...
router.get("/ready", async (req, res) => {
  try {
    ...
    res.render("admin/ready-shipping", { readyOrders: response });
  } catch (err) {
    console.error(err);
  }
});
...
router.get("/processing", async (req, res) => {
  try {
    ...
    res.render("admin/processing-orders", { processingOrders: response });
  } catch (err) {
    console.error(err);
  }
});
...
router.get("/detail/:order_id", async (req, res) => {
  ...
  const data = await SQLquery(`SELECT * FROM \`order\` WHERE ORDER_ID = ? `, [
    order_id,
  ]);
  res.render("admin/order-details", { orderDetail: data[0] });
});
